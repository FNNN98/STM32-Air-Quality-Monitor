...
void read_reg_spi(uint8_t reg, void* buffer, size_t buffer_size) {
    ...
    // Lower CS pin (PA4) to initiate reading transaction
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);
    ...
    if (status != HAL_OK) {
        ...
        // Handle the error as needed, e.g., log the error, reset the device, etc.
    }
    ...
    // Raise CS pin (PA4) to terminate reading transaction
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);
}
